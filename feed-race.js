...
async function mediaDiscovery(pk) {
  ...
  // ── shared classifiers (the same logic the resolver will use) ──────────────
  const kwOf = it => (it.keywordsAll || it.keywords || [])
    .map(k => (k.value || k.displayName || '').toLowerCase()).filter(Boolean);
  ...
  const mp4Of = it => {
    ...
    return m.length ? m[m.length - 1].url : null;
  };
  ...
  try {
    ...
    scoring.slice(-3).forEach(idx => {
      ...
      // ── match by playerId + time proximity, NEVER by headline text ────────
      // the RECAP headline names the batter too, so a headline match hands back
      // the 3-min recap for every play. Exclude recap/condensed, require the
      // batter's playerid tag, take the clip posted SOONEST AFTER the play.
      const batterId = p.matchup?.batter?.id;
      const playT = playAt ? new Date(playAt).getTime() : null;
      const cands = items.filter(it => {
        if (isRecap(it) || isCondensed(it)) return false;
        if (!batterId || !playT) return false;
        if (!kwOf(it).includes('playerid-' + batterId)) return false;
        const t = new Date(it.date || it.timestamp).getTime();
        return t >= playT - 120000;          // 2 min of clock skew allowed
      }).sort((a, b) => new Date(a.date || a.timestamp) - new Date(b.date || b.timestamp));

      const match = cands[0];
      if (match) {
        const t = new Date(match.date || match.timestamp).getTime();
        const lagS = ((t - playT) / 1000).toFixed(0);
        console.log('        CLIP: ' + (match.headline || '').slice(0, 50));
        console.log('        \u2605 CLIP LAG: ' + lagS + 's after the play  (' + cands.length + ' candidate' + (cands.length === 1 ? '' : 's') + ')');
        console.log('          mp4: ' + (mp4Of(match) || 'none').slice(0, 76));
      } else {
        console.log('        \u2717  no clip yet for this play');
      }
    });
  } catch (e) {
  ...
  // ── 3. RESOLVER DRY-RUN — exactly what mlbRecap() would hand the director ──
  console.log('\n' + '─'.repeat(78));
  console.log('RESOLVER DRY-RUN  —  what mlbRecap() returns for this gamePk');
  console.log('─'.repeat(78));
  const recap = items.find(isRecap);
  if (recap) {
    console.log('  title:    ' + (recap.headline || recap.title));
    console.log('  duration: ' + recap.duration);
    console.log('  mp4:      ' + (mp4Of(recap) || 'NO MP4 RENDITION'));
    console.log('  -> this is what drops straight into the <video> tag.');
  } else {
    console.log('  no mlb_recap item -> resolver returns null -> falls through to YouTube.');
  }
  const cond = items.find(isCondensed);
  console.log(cond
    ? '  (condensed game present @ ' + cond.duration + ' - correctly NOT selected)'
    : '  (no condensed game in this payload)');
